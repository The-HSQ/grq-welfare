...
// If you're fetching the image URL from an API and then displaying it
export const fetchImageData = async (originalImageUrl: string) => {
  try {
    // Use proxy when making the request
    const proxiedUrl = proxyUrl + originalImageUrl;
    const response = await axios.get(proxiedUrl, {
      headers: {
        "X-Requested-With": "XMLHttpRequest",
      },
      responseType: "blob", // If you need the actual image data
    });
    ...
    try {
      ...
      if (refreshToken) {
        ...
        await api.post("/auth/logout/", { refresh: refreshToken }).catch(() => {
          ...
          console.log(
            "Logout endpoint not available or failed, proceeding with local logout"
          );
        });
      }
    } catch (error) {
